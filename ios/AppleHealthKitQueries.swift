...
class HealthKitQueries {
  ...
  func getAppleMoveTimeQuery(daysBefore: Int, completion: @escaping ([[String: Any]]?, Error?) -> Void) {
        guard #available(iOS 14.5, *) else {
            let error = NSError(domain: "HealthKit", code: -1, userInfo: [NSLocalizedDescriptionKey: "Method is not allowed for IOS older than 14.5"])
            completion(nil, error)
            return
        }
        ...
        query.initialResultsHandler = { _, results, error in
            ...
            var moveTimeDictionaries: [[String: Any]] = []

            results?.enumerateStatistics(from: startDate, to: endDate) { statistics, _ in
                if let averageMoveTime = statistics.sumQuantity()?.doubleValue(for: HKUnit.minute()) {
                    moveTimeDictionaries.append([
                        "dateString": getShortStringDate(statistics.startDate),
            ...
            do {
                let validDictionaries = try checkIsEmptyArray(moveTimeDictionaries, "No Health Data Available")
                completion(validDictionaries, nil)
            } catch {
                completion(nil, error)
            }
